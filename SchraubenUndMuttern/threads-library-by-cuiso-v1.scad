...
- I have no lag problems in preview(F5) mode during the test of this library. If you are suffering lag problems in preview you can try to un-comment the "render()" al lines 250 and 255.  
...
delta=360/divdelta;
intersection(){
cubointter(long=lenght,pitch=pitch,diameter=diameter);
...
if(entry==1){translate([0,0,-0.001])cubointter_nut_entry(lenght+0.002,pitch,diameter);}
//#cylinder(d=diameter,h=lenght,$fn=100);
}

module cubointter(long,pitch,diameter)
{
longcube=long-pitch;
translate([0,0,longcube/2+pitch/2])cube([diameter,diameter,longcube],center=true);
translate([0,0,longcube+pitch/2])
cylinder(d1=diameter,d2=diameter-2*pitch*0.866,h=pitch/2, $fn=60);
cylinder(d2=diameter,d1=diameter-2*pitch*0.866,h=pitch/2, $fn=60);
}
...
module cubointter_nut_entry(long,pitch,diameter)
{
longcube=long-0.6;
translate([0,0,longcube])
cylinder(d2=diameter+0.4,d1=diameter,h=0.6, $fn=60);
cylinder(d1=diameter+0.4,d2=diameter,h=0.6, $fn=60);
}
